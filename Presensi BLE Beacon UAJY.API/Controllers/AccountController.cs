namespace ShopWeb.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> CreateToken([FromBody] LoginViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = await userHelper.GetUserByEmailAsync(model.NPM);
                if (user != null)
                {
        ...
        [HttpPut]
        public async Task<IActionResult> PutUser([FromBody] Presensi_BLE_Beacon_UAJY.Common.Models.User user)
        {
            if (!ModelState.IsValid)
            {
            ...
            var userEntity = await this.userHelper.GetUserByEmailAsync(user.NPM);
            if (userEntity == null)
            {
            ...
            var updateUser = await this.userHelper.GetUserByEmailAsync(user.NPM);
